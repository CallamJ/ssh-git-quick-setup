...
display_public_keys() {
    ...
    echo -e "${YELLOW}Copy these public keys to your GitHub accounts:${NC}\n"
    for account in "${ACCOUNTS[@]}"; do
        IFS=':' read -r pattern name email <<<"$account"
        local key_name="${pattern//github-/}"
        key_name="${key_name//gitlab-/}"
...
prompt_keys_saved() {
    print_header "Step 10: Add SSH Keys to GitHub"

    if ! [ -t 0 ]; then
        print_warning "Non-interactive shell detected; skipping confirmation prompt."
        return 0
    fi

    while true; do
        read -rp "Have you added all SSH keys to your GitHub accounts? (yes/no): " response
        case "$response" in
        [Yy] | [Yy][Ee][Ss]) return 0 ;;
        *) print_warning "Please add your SSH keys before continuing." ;;
        esac
    done
}

test_ssh_connections() {
    print_header "Step 11: Testing SSH Connections"
    for account in "${ACCOUNTS[@]}"; do
        IFS=':' read -r pattern _ _ <<<"$account"
        print_info "Testing $pattern"
        ssh -T -n "git@$pattern" || true
        echo
    done
}
